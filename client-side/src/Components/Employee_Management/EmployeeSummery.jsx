...
export default function EmployeeSummery(props) {
  ...
  function confirmDelte() {
    Axios.delete(`http://localhost:3001/delete/${empIdForDelete}`).then(
      (res) => {
        if (res.status === 200) {
  ...
  return (
    <>
      ...
      />
      {isAddEmpRender ? <EmpAddModel isAddModuleOpen={isAddEmpRender} /> : null}
      {isViewEmpRender ? (
        <EmpViewModel
      ...
      <div
        id="employeeSummery"
        style={{ display: isEmpSummryShow ? "block" : "none" }}
      >
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <div>
            <h1>Employee Summery</h1>
          </div>
          <div>
            <Button onClick={addEmployee} variant="outline-success" size="lg">
              Add Employee
            </Button>{" "}
          </div>
        </div>
