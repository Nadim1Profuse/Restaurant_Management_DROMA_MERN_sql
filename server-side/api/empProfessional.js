...
routerEmpProf.post("/empProfApi/add", body, (req, res) => {
  ...
  res.send(
    `Successfully Added Employee Professional Detail For EmpId=${lastAddedEmpId}`
  );
});
...
routerEmpProf.post("/empProfApi/update/:profId", (req, res) => {
  ...
  db.query(sql, (err) => {
    if (!err) {
      console.log(
        `succesfully updated employee Professional Detailst where empId=${updatedProfDetail.empId} and addrId=${profId}`
      );
      res.send(
        `succesfully updated employee address/contact where empId=${updatedProfDetail.empId} and addrId=${profId}`
      );
    } else {
      console.log(err);
    }
...
routerEmpProf.post("/empProfApi/delete", (req, res) => {
  ...
  db.query(sql, (err) => {
    if (!err) {
      res.send(
        `suceesfully deleted the employeement_professional_details of emp whre empId=${empId} and profId=${profId} `
      );
      console.log(
        `suceesfully deleted the employeement_professional_details of emp whre empId=${empId} and profId=${profId} `
      );
