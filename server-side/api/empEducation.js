...
routerEmpEdu.post("/empEduApi/add", body, (req, res) => {
  ...
  res.send(
    `successfully added doc in "emp_education_details" table where empId=${lastAddedEmpId}`
  );
});
...
routerEmpEdu.post("/empEduApi/update/:educnId", (req, res) => {
  ...
  db.query(sql, (err) => {
    if (!err) {
      console.log(
        `succesfully updated employee Education Detailst where empId=${updatedEducationDetail.empId} and educnId=${educnId}`
      );
      res.send(
        `succesfully updated employee Educationt where empId=${updatedEducationDetail.empId} and educnId=${educnId}`
      );
    } else {
      console.log(err);
    }
...
routerEmpEdu.post("/empEduApi/delete", (req, res) => {
  ...
  db.query(sql, (err) => {
    if (!err) {
      res.send(
        `suceesfully deleted the emp_Education_Details of emp whre empId=${empId} and educnId=${educnId} `
      );
      console.log(
        `suceesfully deleted the emp_Education_Details of emp whre empId=${empId} and educnId=${educnId} `
      );
