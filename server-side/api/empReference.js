...
routerEmpRefer.post("/empReferApi/add", body, (req, res) => {
  ...
  empReferenceArray.map((emp) => {
    ...
    db.query(sql, post, (err) => {
      if (!err) {
        console.log(
          "successfully inserted docs into `employeement_professional_details` table"
        );
      } else {
        console.log(err);
      }
...
routerEmpRefer.post("/empReferApi/update/:refId", (req, res) => {
  ...
  db.query(sql, (err) => {
    if (!err) {
      console.log(
        `succesfully updated emp_reference_details Detailst where empId=${updatedRefrnceDetail.empId} and refId=${refId}`
      );
      res.send(
        `succesfully updated emp_reference_details where empId=${updatedRefrnceDetail.empId} and refId=${refId}`
      );
    } else {
      console.log(err);
    }
...
routerEmpRefer.post("/empReferApi/delete", (req, res) => {
  ...
  db.query(sql, (err) => {
    if (!err) {
      res.send(
        `suceesfully deleted the employeement_professional_details of emp whre empId=${empId} and refId=${refId} `
      );
      console.log(
        `suceesfully deleted the emp_reference_details of emp whre empId=${empId} and refId=${refId} `
      );
    } else {
      console.log(err);
    }
